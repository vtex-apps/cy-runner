...
module.exports.vtexTestStrategy = async (config) => {
    ...
    if (test.enabled) {
      ...
      if (typeof dependency != 'undefined') {
        ...
        if (check.length === dependency.length) {
          qe.msg(`As testStrategy [${check}] passed, running [testStrategy.${strategy}]`)
          await runTest(test, config, group)
        } else {
          qe.msg(`One of its dependencies [${dependency}] not pass, skipping [testStrategy.${strategy}]`)
  ...
  return {time: qe.toc(START), testsFailed: testsFailed, testsSkipped: testsSkipped, testsPassed: testsPassed}
}
...
async function runTest(test, config, group) {
  ...
  for (let ht = 0; ht <= test.hardTries; ht++) {
    if (!testPassed) {
      qe.msg(`Running try [${ht + 1}] of [${test.hardTries + 1}] for [testStrategy.${test.name}]`)
      if (test.runInOrder) {
        let passSpec = []
  ...
  if (!testPassed) {
    ...
    if (test.stopOnFail) await qe.stopOnFail(config, `testStrategy.${test.name}`)
  } else {
